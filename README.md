...
[![test](https://github.com/beyto1974/cultuurconnect-catalogus-openapi/actions/workflows/test.yml/badge.svg)](https://github.com/beyto1974/cultuurconnect-catalogus-openapi/actions/workflows/test.yml)
...
The first badge is served by GitHub and reflects the latest `test` workflow run on the default
branch; while this repository is private it only renders for signed-in users who have access.
...
The MIT licence in `LICENSE` covers **this repository's own work**: `openapi.json`, the test
suites, the badge script and the CI configuration.

It does not and cannot cover
`cultuurconnect-api-catalogus-webservices-v1-publieke-documentatie.md`, which is
Cultuurconnect's document, included here as the source this spec was reconstructed from (with
the API keys and two employees' personal addresses replaced). Nor does it cover the contents of
`samples/`, which are catalogue records served by the API and remain subject to Cultuurconnect's
terms — the rich API's third-party data in particular. This project is not affiliated with or
endorsed by Cultuurconnect.
...
* The test suite redacts the key out of returned URLs and failure messages, because every
  request carries it as `?authorization=…` and CI uploads `coverage/` as a build artifact.
  GitHub masks registered secrets in log output but not inside artifact files, so the workflow
  additionally greps the artifact and fails rather than uploading if the key ever appears.
...
Published on GitHub Pages, rebuilt on every change to the spec:

| | |
|---|---|
| Human-readable reference | <https://beyto1974.github.io/cultuurconnect-catalogus-openapi/> |
| The spec itself | <https://beyto1974.github.io/cultuurconnect-catalogus-openapi/openapi.json> |
| Captured samples | <https://beyto1974.github.io/cultuurconnect-catalogus-openapi/samples/> |

The spec URL is served with `Access-Control-Allow-Origin: *`, so it can be pasted straight into
Swagger Editor, Postman, or a client generator.
...
Both workflows run on GitHub-hosted runners, which are free for public repositories.
`pages.yml` builds the reference and deploys it to GitHub Pages.
